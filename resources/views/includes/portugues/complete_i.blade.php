<div class="container-fluid materias-container text-uppercase">
    <div class="col-sm-12 col-xs-12 line-task">
        <div class="col-sm-6 col-xs-6 line-margin-left">
            <div class="check-img-tbl-left col-sm-1 col-xs-1">
                <i class="hidden icon-atv-check icon-atv-check-tbl icon-atv-check-8 fa fa-check fa-3x margin-bottom"></i>
                <i class="hidden icon-atv-error icon-atv-check-tbl icon-atv-error-8 fa fa-times fa-3x margin-bottom"></i>
            </div>
            {!! Html::image('/assets/img/portugues/igreja.png', 'igreja', ['class' => 'img-atividade img-atividade-tbl img-atividade-a']) !!}
            <div class="table-complete">
                <table class="table-border table-responsive">
                    <tr>
                        <td class="text-center">
                            <input type="text" class="task_input task_input-i" id="I" maxlength="1" />
                        </td>
                        <td class="text-center">
                            <input type="text" class="task_input task_input-i" id="G"  readonly value="G" maxlength="1" />
                        </td>
                        <td>
                            <input type="text" class="task_input task_input-i" id="R"  readonly value="R" maxlength="1" />
                        </td>
                        <td>
                            <input type="text" class="task_input task_input-i" value="E"  readonly id="E" maxlength="1" />
                        </td>
                        <td>
                            <input type="text" class="task_input task_input-i" id="J"  readonly value="J" maxlength="1" />
                        </td>
                        <td>
                            <input type="text" class="task_input task_input-i" value="A"  readonly id="A" maxlength="1" />
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <input type="text" class="task_input task_input-i" id="I" maxlength="1" />
                        </td>
                        <td>
                            <input type="text" class="task_input task_input-i" id="G" maxlength="1" />
                        </td>
                        <td>
                            <input type="text" class="task_input task_input-i" id="R" maxlength="1" />
                        </td>
                        <td>
                            <input type="text" class="task_input task_input-i" id="E" maxlength="1" />
                        </td>
                        <td>
                            <input type="text" class="task_input task_input-i" id="J" maxlength="1" />
                        </td>
                        <td>
                            <input type="text" class="task_input task_input-i" id="A" maxlength="1" />
                        </td>
                    </tr>
                </table>
            </div>
        </div>
        <div class="col-sm-6 col-xs-6 line-margin-right">
            {!! Html::image('/assets/img/portugues/ioio.png', 'ioio', ['class' => 'img-atividade img-atividade-a img-atividade-tbl']) !!}
            <div class="table-complete">
                <table class="table-border table-responsive">
                    <tr>
                        <td class="text-center">
                            <input type="text" class="task_input task_input-i" id="I" maxlength="1" />
                        </td>
                        <td>
                            <input type="text" class="task_input task_input-i" value="O"  readonly id="O" maxlength="1" />
                        </td>
                        <td>
                            <input type="text" class="task_input task_input-i" id="I" maxlength="1" />
                        </td>
                        <td>
                            <input type="text" class="task_input task_input-i" value="O"  readonly id="O" maxlength="1" />
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <input type="text" class="task_input task_input-i" id="I" maxlength="1" />
                        </td>
                        <td>
                            <input type="text" class="task_input task_input-i" id="O" maxlength="1" />
                        </td>
                        <td>
                            <input type="text" class="task_input task_input-i" id="I" maxlength="1" />
                        </td>
                        <td>
                            <input type="text" class="task_input task_input-i" id="O" maxlength="1" />
                        </td>
                    </tr>
                </table>
                <table class="check-img-tbl-right">
                    <tr>
                        <td>
                            <div class="col-sm-1 col-xs-1">
                                <i class="hidden icon-atv-check icon-atv-check-tbl icon-atv-check-8 fa fa-check fa-3x margin-bottom"></i>
                                <i class="hidden icon-atv-error icon-atv-check-tbl icon-atv-error-8 fa fa-times fa-3x margin-bottom"></i>
                            </div>
                        </td>
                    </tr>
                </table>
            </div>
        </div>
    </div>
    <div class="col-sm-12 col-xs-12 line-task">
        <div class="col-sm-6 col-xs-6 line-margin-left">
            <div class="check-img-tbl-left col-sm-1 col-xs-1">
                <i class="hidden icon-atv-check icon-atv-check-tbl icon-atv-check-8 fa fa-check fa-3x margin-bottom"></i>
                <i class="hidden icon-atv-error icon-atv-check-tbl icon-atv-error-8 fa fa-times fa-3x margin-bottom"></i>
            </div>
            {!! Html::image('/assets/img/portugues/indio.png', 'indio', ['class' => 'img-atividade img-atividade-a img-atividade-tbl']) !!}
            <div class="table-complete">
                <table class="table-border table-responsive">
                    <tr>
                        <td class="text-center">
                            <input type="text" class="task_input task_input-i" id="I" maxlength="1" />
                        </td>
                        <td>
                            <input type="text" class="task_input task_input-i"  readonly value="N" id="N" maxlength="1" />
                        </td>
                        <td>
                            <input type="text" class="task_input task_input-i"  readonly value="D" id="D" maxlength="1" />
                        </td>
                        <td>
                            <input type="text" class="task_input task_input-i"  id="I" maxlength="1" />
                        </td>
                        <td>
                            <input type="text" class="task_input task_input-i"  readonly value="O" id="O" maxlength="1" />
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <input type="text" class="task_input task_input-i" id="I" maxlength="1" />
                        </td>
                        <td>
                            <input type="text" class="task_input task_input-i" id="N" maxlength="1" />
                        </td>
                        <td>
                            <input type="text" class="task_input task_input-i" id="D" maxlength="1" />
                        </td>
                        <td>
                            <input type="text" class="task_input task_input-i" id="I" maxlength="1" />
                        </td>
                        <td>
                            <input type="text" class="task_input task_input-i" id="O" maxlength="1" />
                        </td>
                    </tr>
                </table>
            </div>
        </div>
        <div class="col-sm-6 col-xs-6 line-margin-right">
            {!! Html::image('/assets/img/portugues/ilha.png', 'ilha', ['class' => 'img-atividade img-atividade-a img-atividade-tbl']) !!}
            <div class="table-complete">
                <table class="table-border table-responsive">
                    <tr>
                        <td class="text-center">
                            <input type="text" class="task_input task_input-i" id="I" maxlength="1" />
                        </td>
                        <td>
                            <input type="text" class="task_input task_input-i" value="L"  readonly id="L" maxlength="1" />
                        </td>
                        <td>
                            <input type="text" class="task_input task_input-i"  readonly value="H" id="H" maxlength="1" />
                        </td>
                        <td>
                            <input type="text" class="task_input task_input-i" value="A"  readonly id="A" maxlength="1" />
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <input type="text" class="task_input task_input-i" id="I" maxlength="1" />
                        </td>
                        <td>
                            <input type="text" class="task_input task_input-i" id="L" maxlength="1" />
                        </td>
                        <td>
                            <input type="text" class="task_input task_input-i" id="H" maxlength="1" />
                        </td>
                        <td>
                            <input type="text" class="task_input task_input-i" id="A" maxlength="1" />
                        </td>
                    </tr>
                </table>
                <table class="check-img-tbl-right">
                    <tr>
                        <td>
                            <div class="col-sm-1 col-xs-1">
                                <i class="hidden icon-atv-check icon-atv-check-tbl icon-atv-check-8 fa fa-check fa-3x margin-bottom"></i>
                                <i class="hidden icon-atv-error icon-atv-check-tbl icon-atv-error-8 fa fa-times fa-3x margin-bottom"></i>
                            </div>
                        </td>
                    </tr>
                </table>
            </div>
        </div>
    </div>
</div>
@include('includes.scripts')
<script>
    $(document).ready(function () {

        var erros = 0;
        var acertos = 0;
        var $dateIni = getDate();

        $('.btn-atividade-11').on('click', function(e){
            console.log(erros);
            console.log(acertos);
            $('.task_input-i').each(function(e){
                if(!$(this).is('[readonly]')){
                    var letra = $(this).attr('id');
                    if($(this).val().toUpperCase() == letra){
                        if(!$(this).hasClass('acerto-tbl')){
                            $("#acertos-11").val(acertos++);
                        }
                        $(this).addClass('no-click');
                        $(this).attr('readonly', 'true');
                        $(this).unbind('click');
                        $(this).addClass('acerto-tbl');
                        if(!$(this).parent('table').find('.task_input-i').hasClass('erro-tbl')){
                            $(this).parent('table').parent('div').find('.icon-atv-error').addClass('hidden');
                            $(this).parent('table').parent('div').find('.icon-atv-check').removeClass('hidden');
                        }
                    } else if($(this).val().toUpperCase() != letra){
                        if($(this).val() != ''){
                            swal("Ops... Alguma resposta está errada!", "", "warning");
                            playSound('wrong');
                            $("#erros-11").val(erros++);
                            $(this).addClass('erro-tbl');
                            $(this).parent('table').parent('div').find('.icon-atv-check').addClass('hidden');
                            $(this).parent('table').parent('div').find('.icon-atv-error').removeClass('hidden');
                        } else if($(this).val() == ''){
                            swal("Ops... Atividade incompleta!", "", "warning");
                            playSound('wrong');
                        }
                    }
                }
            });
            if (acertos >= 25){
                playSound('claps');
                swal({
                            title: "Parabéns, você acertou todas as respostas!",
                            type: "success",
                            showCancelButton: false,
                            closeOnConfirm: true
                        },
                        function (isConfirm) {
                            if (isConfirm) {
                                var sbtvId = $("#hdn_subatividade_id-11").val();
                                var toHide = '.subatividade-11';
                                var toShow = '.subatividade-12';
                                var erros = $("#erros-11").val();
                                var acertos = $("#acertos-11").val();
                                var dateFim = getDate();
                                var no_errors = false;

                                if(erros == 0){
                                    no_errors = true;
                                }
                                minTry(sbtvId, 24, acertos, erros, toHide, toShow, 5, $dateIni, dateFim, no_errors);
                            }
                        });
            }
        });

        $('#btn-prev-11').on('click', function(e){
            var toHide = '.subatividade-11';
            var toShow = '.subatividade-10';
            btnPrevAtvd(toHide, toShow);
        });
    });
</script>